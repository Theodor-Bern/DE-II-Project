#!/usr/bin/env python3
"""
Commit aggregator: subscribes to `commit-topic`, keeps the top-N repos
by commit count, and periodically logs them.

Answers project question Q2: most frequently updated GitHub projects.
"""
import os
import json
import pulsar
...
def render_top(repos_dict, n):
    """Format top-N repos as a string."""
    ...
    for i, repo in enumerate(sorted_repos[:n], 1):
        ...
        lines.append(f"  {i:2d}. {repo['full_name']:<50s} {repo['commit_count']:>8d} commits  [{lang}]")
    return "\n".join(lines)
...
def main():
    ...
    # Keyed by repo_id to deduplicate if a message is redelivered
    repos = {}
    seen = 0
